...
module.exports = (async () => {
  try {
    const TYPESENSE_CONFIG: ConfigurationOptions = {
      nodes: [
        {
          host: process.env.TYPESENSE_ADMIN_HOST,
          port: +process.env.TYPESENSE_ADMIN_PORT,
          protocol: process.env.TYPESENSE_ADMIN_PROTOCOL,
        },
      ],
      ...
      for (let i = 0; i < languages.length; i++) {
        ...
        try {
          ...
          if (process.argv[2] === '--delete') {
            console.log('Removing collection and genberating schema again');
